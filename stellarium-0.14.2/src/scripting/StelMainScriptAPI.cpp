...
QVariantMap StelMainScriptAPI::getSelectedObjectInfo()
{
	...
	map.insert("type", obj->getType()); //PLANETC_GC
	map.insert("AllInfo", obj->getInfoString(core, StelObject::InfoStringGroup(StelObject::AllInfo))); //PLANETC_GC
...
void StelMainScriptAPI::moveToObject(float duration) {
        ...
        mvmgr->moveToObject(obj, duration, 1);
}
