...
void SearchDialog::gotoObject(const QString &nameI18n)
{
	...
	//PLANETC_GC StelMovementMgr* mvmgr = GETSTELMODULE(StelMovementMgr);
	if (simbadResults.contains(nameI18n))
	{
		if (objectMgr->findAndSelect(nameI18n))
		{
			...
			if (!newSelected.empty())
			{
				...
				{
					//PLANETC_GC mvmgr->moveToObject(newSelected[0], mvmgr->getAutoMoveDuration());
					//PLANETC_GC mvmgr->setFlagTracking(true);
				}
				else
				{
